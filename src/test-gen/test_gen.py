...
class TestFileGeneratorBase():
    ...
    def __get_c_data_type(self, data_type):
        if data_type.startswith("int") and self._get_c_data_width(data_type) <= 8:
            return "int8_t"
        elif data_type.startswith("uint") and self._get_c_data_width(data_type) <= 8:
            return "uint8_t"
...
class BitwiseTestGenerator(TestFileGeneratorBase):
    ...
    def generate(self):
        ...
        # Note: Use C++ style code for test_bitwise.cpp
        ...
        random_number_generation_string = self._get_random_number_generation_string(c_style=False)
...
bool runTest({inputs_string_with_type}) {{
    ...
    {self._get_verification_code_string(c_style=False)}
...
int main() {{
  ...
  PimStatus status = pimCreateDevice({pim_device}, 1, 1, 2, 16384, 1024);
  if (status != PIM_OK) {{
